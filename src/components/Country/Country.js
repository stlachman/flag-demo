...
const Country = props => {
  ...
  useEffect(() => {
    const fetchCountry = name => {
      axios
        .get(
          `https://restcountries.eu/rest/v2/alpha/${name}?fields=name;flag;nativeName;population;region;subregion;capital;topLevelDomain;currencies;languages;borders`
        )
        .then(res => {
          setCountry(res.data);
          setCode({ abbreviations: res.data.borders });
        });
    };
    fetchCountry(id);
  }, [id]);
